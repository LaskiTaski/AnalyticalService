...
class BondResponse(BaseModel):
    ...
    # Issuer / risk signals (этап 1)
    issuer_inn: str | None = None
    risk_events_count: int = Field(default=0, description="Число риск-событий эмитента")
    has_severe_events: bool = Field(
        default=False, description="Есть тяжёлые события: default/tech_default/bankruptcy_intent/restructuring"
    )
...
class BondFilterParams(BaseModel):
    ...
    security_type: str | None = Field(
        default=None, description="Bond type(s), comma-separated: ofz,corp,muni"
    )
    board_id: str | None = Field(default=None, description="Board: TQCB, TQOB, TQIR")

    # Search
    search: str | None = Field(
        default=None, max_length=100, description="Search by name / SECID / ISIN"
    )

    # Risk signals
    risk_only: bool = Field(
        default=False, description="Только бумаги эмитентов с риск-событиями"
    )
...
class IssuerEventResponse(BaseModel):
    """Риск-событие эмитента."""
...
class IssuerEventsResponse(BaseModel):
    """Лента событий эмитента."""

    inn: str
    items: list[IssuerEventResponse]
    total: int
